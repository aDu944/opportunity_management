...
def on_quotation_after_insert(doc, method=None):
    ...
    on the linked Opportunity + Sales Managers."""
    if doc.docstatus != 0:
        return
    opp = doc.get("opportunity")
    recipients = _opportunity_responsible_users(opp) + _users_with_role("Sales Manager")
    _dispatch(recipients, T.quotation_created, doc)


def on_quotation_submit(doc, method=None):
    """Quotation was submitted (docstatus 1)."""
    recipients = _opportunity_responsible_users(doc.get("opportunity")) + _users_with_role("Sales Manager")
...
def on_sales_order_submit(doc, method=None):
    recipients = _users_with_role("Sales Manager") + _users_with_role("Sales User")
...
def on_sales_invoice_submit(doc, method=None):
    recipients = _users_with_role("Accounts Manager") + _users_with_role("Accounts User")
    if doc.owner:
        recipients.append(doc.owner)
...
def on_material_request_submit(doc, method=None):
    recipients = _users_with_role("Stock Manager") + _users_with_role("Purchase Manager")
    _dispatch(recipients, T.material_request_submitted, doc)


def on_delivery_note_submit(doc, method=None):
    recipients = _users_with_role("Sales Manager") + _users_with_role("Stock Manager")
    if doc.owner:
        recipients.append(doc.owner)
